...
@pytest.mark.parametrize("database_url", DATABASE_URLS)
@async_adapter
async def test_queries_bindparams(database_url):
    async with Database(database_url) as database:
        # async with database.transaction(force_rollback=True):
        ...
        assert results[0]["text"] == "example22"
        assert results[0]["completed"] == False
        assert results[1]["text"] == "example32"
        assert results[1]["completed"] == True
        assert [r['text'] for r in results] == ['example22', 'example32']

        # execute_many with bindparans
        stmt = notes.update().where(notes.c.id==bindparam("b_id")).values({"text":bindparam("b_text"), "completed":bindparam("b_completed")})
        values = [
            {"b_text": "example22_update", "b_completed": True, "b_id": 1},
            {"b_text": "example32_update", "b_completed": False, "b_id": 2},
        ]
        # await database.execute_many(stmt, values)
        ...
        results = await database.fetch_all(query=query)
        assert len(results) == 2
        assert results[0]["text"] == "example22_update"
        assert results[0]["completed"] == True
        assert results[1]["text"] == "example32_update"
        assert results[1]["completed"] == False
